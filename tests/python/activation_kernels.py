...
def call_uninterp(left, right):
    return Call(UNINTERP, Int(), left, right)
...
def target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    ...
    def uninterp_body(x, y):
        return call_uninterp(UNINTERP, Int(), x, y)
    uninterp = FnDeclRecursive(UNINTERP, Int(), uninterp_body(x, y), x, y)

    return [vector_add, scalar_mul, uninterp]
...
def ps_grammar(writes: List[Var], reads: List[Var]) -> Expr:
    ...
    return Implies(And(Eq(ml_list_length(base), ml_list_length(active)), Gt(ml_list_length(base), IntLit(0))),
        Call("list_eq", Bool(), ret_val, call_vector_add(call_scalar_mul(opacity, active), call_scalar_mul(Sub(IntLit(255), opacity), base))))
                       ...
                       And(Ge(i, IntLit(0)),
                           ...
                           Eq(agg_result,
                              call_vector_add(call_scalar_mul(opacity, ml_list_take(active, i)),
                                    call_scalar_mul(Sub(IntLit(255), opacity), ml_list_take(base, i)))
                              )))
...
if __name__ == "__main__":
    ...
    uninterp_fns = [UNINTERP]

    driver = Driver()
    activation_kernels = driver.analyze(
    ...
    x_var = driver.variable("x", ListT(Int()))
    y_var = driver.variable("y", ListT(Int()))
    driver.uninterp_fns = uninterp_fns

    activation_kernels(x_var, y_var)
    driver.synthesize(noVerify=True)

    print("\n\ngenerated code:" + activation_kernels.codegen(codegen))
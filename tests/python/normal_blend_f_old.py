...
from metalft.frontend.llvm import Driver
...
def ml_vector_add(left, right):
    return Call(VECTORADD, ListT(Int()), left, right)

def ml_scalar_mul(left, right):
...
def targetLang(kernel_size=2):
    ...
    #def blend_body(opacity, active, base):
    #    return Add(Mul(opacity, active), Mul(Sub(IntLit(10), opacity), base))
    #blend = FnDecl(BLEND, Int(), blend_body(a, b), a, b)

    #def normalblend_body(base, active, opacity)
    #    vec_size = ml_list_length(base)
    #    cur = ml_blend(opacity, ml_list_head(active), ml_list_head(base))
    #    base_rest = ml_list_tail(base, IntLit(1))
    #    active_rest = ml_list_tail(active, IntLit(1))
    #    recursed = ml_normalblend(base_rest, active_rest, opacity)
    #    general_answer = ml_list_prepend(cur, recursed)
    #    return Ite(Lt(vec_size, IntLit(1)), ml_list_empty(), general_answer)
    #normalblend = FnDeclRecursive(NORMALBLEND, ListT(Int()), normalblend_body(x, y, a), x, y, a)
